...
export default function CareerBankPage() {
  ...
  // New UI states
  const [theme, setTheme] = useState<"cozy" | "light">("cozy")
  const [quickViewCareer, setQuickViewCareer] = useState<Career | null>(null)
  ...
  useEffect(() => {
    // apply theme to document root (so CSS variables switch)
  ...
  const toggleBookmark = useCallback(
    (careerId: string) => {
      ...
      const newBookmarks = isBookmarked ? bookmarkedCareers.filter((id) => id !== careerId) : [careerId, ...bookmarkedCareers]

      setBookmarkedCareers(newBookmarks)
      localStorage.setItem("bookmarkedCareers", JSON.stringify(newBookmarks))
      setToastMessage(isBookmarked ? "Removed bookmark" : "Saved to bookmarks")

      toast({
        title: isBookmarked ? "Bookmark removed" : "Bookmarked",
        description: isBookmarked ? "Career removed from your bookmarks." : "Career saved to your bookmarks.",
        duration: 2500,
      })
    },
  ...
  const openQuickView = (career: Career) => {
    setQuickViewCareer(career)
  }

  const closeQuickView = () => setQuickViewCareer(null)

  const toggleTheme = () => setTheme(theme === "cozy" ? "light" : "cozy")
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Quick View Modal */}
      {quickViewCareer && (
        <div className="quick-view-backdrop" role="dialog" aria-modal="true">
          <div className="quick-view-panel card-cozy p-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-xl font-semibold">{quickViewCareer.title}</h2>
                <p className="text-sm text-muted-foreground">{quickViewCareer.industry} • {quickViewCareer.location?.[0]}</p>
              </div>
              <div className="flex items-center gap-2">
                <Badge className="ribbon">{quickViewCareer.demandLevel} demand</Badge>
                <Button variant="ghost" size="sm" onClick={closeQuickView}><X className="h-4 w-4" /></Button>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-muted-foreground mb-3">{quickViewCareer.description}</p>
                <div className="space-y-2 text-sm">
                  <div><strong>Skills:</strong> {quickViewCareer.skills.join(", ")}</div>
                  <div><strong>Education:</strong> {quickViewCareer.education.join(", ")}</div>
                  <div><strong>Growth Rate:</strong> {quickViewCareer.growthRate}</div>
                </div>
              </div>
              <div>
                <div className="mb-4">
                  <div className="flex justify-between text-sm text-muted-foreground mb-2">
                    ...
                    <span>${quickViewCareer.salaryRange.min.toLocaleString()} - ${quickViewCareer.salaryRange.max.toLocaleString()}</span>
                  </div>
                  <Progress value={(quickViewCareer.salaryRange.max / 100000) * 100} className="h-2" />
                </div>
                <div className="flex gap-2">
                  <Button className="btn-cozy-primary" onClick={() => { markAsViewed(quickViewCareer.id); closeQuickView(); }}>
                    Open Full Page
                  </Button>
                  <Button variant="ghost" onClick={() => { toggleBookmark(quickViewCareer.id); }}>
                    {bookmarkedCareers.includes(quickViewCareer.id) ? 'Bookmarked' : 'Save'}
                  </Button>
                </div>
              </div>
            </div>
      ...
      {/* Back to top */}
      {showBackToTop && (
        <Button className="back-to-top" onClick={scrollToTop} size="sm">
          <ChevronUp className="h-4 w-4" />
        </Button>
      )}

      {/* Toast */}
      {toastMessage && <div className="career-toast">{toastMessage}</div>}
    </div>
  )
}
